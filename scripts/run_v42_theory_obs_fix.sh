#!/bin/bash
# V42: Theory-grounded 23-dim (LOS az/el rates + alignment cos) observation (newswarm.tex)
...
echo "=========================================="
echo " V42: Theory-grounded 23-dim (LOS az/el rates + alignment cos) obs + MAPPO"
echo " Start: $(date)"
echo "=========================================="

conda run --no-capture-output -n rlgpu python -u scripts/train_fov_penetration_mappo.py \
    --algorithm_name mappo \
    --experiment_name v42_theory_obs_fix \
    --scenario scenario_1 \
    --ap_config v28 \
    --seed 42 \
    --cuda \
    ...
    2>&1 | tee outputs/v42_theory_obs_fix.log
