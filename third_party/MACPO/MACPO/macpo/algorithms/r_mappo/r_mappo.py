"""Standard MAPPO (Multi-Agent PPO) Trainer — no cost constraint.

This is the pure reward-optimization PPO trainer used by `base_runner.py`.
Unlike R_MACTRPO_CPO, this class:
  - Has NO cost critic
  - Uses standard clipped PPO policy update (not TRPO/CPO)
  - Only optimises reward-based advantages
"""
...
class R_MAPPO:
    """
    Trainer class for MAPPO (PPO-Clip) to update policies.
    :param args: (argparse.Namespace) arguments.
    :param policy: (R_MAPPOPolicy) policy to update.
    ...
    def ppo_update(self, sample, update_actor=True):
        """
        Update actor and critic networks using PPO-Clip.
        :param sample: (Tuple) data batch.
        :param update_actor: (bool) whether to update actor network.

        :return value_loss, critic_grad_norm, policy_loss, dist_entropy,
                actor_grad_norm, imp_weights
        """
        # Unpack sample — may be 12 or 13 elements depending on whether factor is available
        if len(sample) == 13:
        ...
        if factor_batch is not None:
            surr1 = ratio * factor_batch * adv_targ
            surr2 = torch.clamp(ratio, 1.0 - self.clip_param, 1.0 + self.clip_param) * factor_batch * adv_targ
        else:
