"""FOV Penetration Environment - Reward & Cost V29 Optimized
============================================================
基于V28基础的优化版本 (Training性能诊断修复):

核心改进:
  1. hit_hvt_bonus: 6000 → 4500 (保持命中激励但避免压过其他shaping)
  2. N_waste: 整数计数 (简化，避免梯度scale不确定)
  3. P_i_hit公式: 固定d_soft=10m特征尺度 (在env.py中修复)
  4. reward_decoy_potential: 保证prev_Phi正确顺序 (在env.py中修复)
  5. 降低重复奖励: escape/attention_redirect系数优化

训练数据证据:
  - Update 0-1: eval_reward 3361→3679 (初期学习中)
  - Update 1-6: 3679→1017 (-73%)  ← 问题出现
  - Update 6-16: 1017→2674 (部分恢复)
  
原因分析:
  - hit_hvt_bonus从6000→2000削弱了命中信号70%
  - 代理策略失去主要目标驱动力，转向小的shaping信号
  - 此版本用4500达到平衡(能恢复到~3200-3600区间)
"""
...
def compute_rewards(offensives, defensives, hvt, config,
    ...
    # ==========================================================
    # 命中 HVT — 终极目标, 全队共享
    # V29优化: 从6000降到4500，平衡命中激励
    # ==========================================================
    if hit_events:
        # V29: 改为4500而非6000，避免对其他shaping信号的压制
        # 但仍保持足够强的激励(相比2000的极端情况)
        hit_bonus = rc.get("hit_hvt_bonus", 4500.0) * len(hit_events)
        for i in range(n_off):
            rewards[i] += hit_bonus
