...
class FOVPenetrationEnv:
    ...
    def _compute_space_dims(self):
        ...
        # self: 9, hvt: 3, def: 7*K, teammates: 7*(n_off-1), exposure: 3, global: 2
        self.obs_dim = 9 + 3 + 7 * self.obs_k_def + 7 * (n_off - 1) + 3 + 2
        # all_off: 10*n_off, all_def: 7*n_def, hvt: 3, relations: 4
        self.share_obs_dim = 10 * n_off + 7 * n_def + 3 + 4
    ...
    def step(self, actions):
        ...
        # 4. 击杀与命中 — 记录本步新死亡
        alive_before = [off.alive for off in self.offensives]
        ...
        # 计算本步刚刚死亡的列表
        just_killed = [alive_before[i] and not self.offensives[i].alive
        ...
        rewards_list, reward_info = compute_rewards(
            ...
            self.prev_dists_to_hvt, step_hits, self.current_step,
