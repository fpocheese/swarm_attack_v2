"""FOV Penetration Environment - Reward & Cost V34
===================================================
V34: 从V28/V33基础上修复的重大bug + 奖励尺度重建

V33失败根因: V33更改写入了reward_cost_v28.py，但env实际导入的是
reward_cost.py (本文件，当时为V29 Optimized)，V33指数放大从未生效。

V34核心修复:
  1. 修复wrong-file bug: 直接在reward_cost.py (本文件) 中应用所有改进
  2. 修复奖励尺度失衡: approach_reward per step (0.02) << risk_penalty (0.23)
     - 解决方案: 减小approach归一化距离 obs_range(2500) → approach_norm_dist(500)
     - 效果: approach_reward提升5倍, 从0.02→0.1+
  3. 新增per-step proximity drive: 直接惩罚远离HVT的状态
     - lambda_proximity * (d/D_max), 在1500m处≈0.22/step, 与risk同量级
     - 确保'接近目标'在任何距离都有明确梯度
  4. 保留V33指数近距放大 (500m以内quadratic到20x)
  5. 进一步降低risk惩罚, 让approach信号主导
"""
...
# ======================================================================
# Main: compute_rewards (V29 Optimized)
# ======================================================================
def compute_rewards(offensives, defensives, hvt, config,
    ...
    ap_data: dict, 由 env.step 中的 analytic priors 模块计算后传入
    """
    rc = config["reward"]
    ...
    # V34: 用approach_norm_dist(500)替代obs_range(2500)作为归一化距离
    #       使approach_reward提升5倍, 与risk penalty同量级
    ...
    total_hit_geo = 0.0
    # V34: 归一化距离 — 关键修复, 从2500→500
    approach_norm_dist = rc.get("approach_norm_dist", 500.0)
    # V33/V34: 近距指数放大参数
    ...
    for i, off in enumerate(offensives):
        if off.alive and not off.hit_hvt and prev_dists_to_hvt[i] < float('inf'):
            # (a) 距 HVT 更近 — V34: 归一化用 approach_norm_dist
            delta_rho = prev_dists_to_hvt[i] - cur_dists[i]
            approach_r = lambda_rho * delta_rho / approach_norm_dist

            # V34/V33: 近距指数放大 — 越近HVT奖励越强
            d = cur_dists[i]
            if d < close_range_threshold:
                # quadratic放大: d从500→0时, 倍率从1→close_range_max_mult
                t = max(1.0 - d / close_range_threshold, 0.0)
                multiplier = 1.0 + (close_range_max_mult - 1.0) * (t ** 2)
            else:
                # 远距: 线性放大 (用obs_range归一化)
                obs_rng = max(config.get("obs_range", 2500.0), 1.0)
                dist_ratio = max(1.0 - d / obs_rng, 0.0)
    ...
    # --- 3.1.4 V34新增: per-step proximity drive (距离惩罚) ---
    # 直接惩罚远离HVT的状态, 越远惩罚越大, 创造持续梯度
    # 在d=1500m处: penalty≈0.22/step, 与risk penalty(0.23)同量级
    lambda_proximity = rc.get("lambda_proximity", 0.3)
    proximity_norm = rc.get("proximity_norm_dist", 2000.0)
    total_proximity_pen = 0.0
    for i, off in enumerate(offensives):
        if off.alive and not off.hit_hvt:
            prox_pen = lambda_proximity * (cur_dists[i] / proximity_norm)
            rewards[i] -= prox_pen
            total_proximity_pen += prox_pen
    reward_info["penalty_proximity"] = total_proximity_pen
    ...
    # --- 3.2.3 reward_attention_redirect: V29优化 - 降低系数避免重复 ---
    lambda_R = rc.get("lambda_attention_redirect", 0.0)  # V29: 改为0，避免与U_decoy重复
    ...
    # --- 3.3.1 reward_escape: V29优化 - 降低系数，P_pen已包含E_esc ---
    lambda_E = rc.get("lambda_escape", 0.1)  # V29: 0.5→0.1 避免重复
    ...
    lambda_dE = rc.get("lambda_escape_progress", 0.1)  # V29: 0.3→0.1
