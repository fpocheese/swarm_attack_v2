...
DEFAULT_CONFIG = {
    ...
    "reward": {
        "hit_hvt_bonus": 500.0,           # 命中HVT: 巨额 (终极目标)
        "approach_hvt_coef": 500.0,        # 接近HVT: 极强 (每步约+0.50)
        "progress_coef": 0.5,              # 进度奖励: 靠近就给 (每步+0.05~0.50)
        "retreat_penalty": -0.30,          # 后退惩罚: 不接近就重扣
        "detected_penalty": -0.01,         # 被探测: 极轻 (只是提醒)
        "fov_escape_bonus": 2.0,           # 脱离FOV: 即时奖励
        "killed_penalty": -20.0,           # 被击杀: 适度 (只触发一次)
        "sacrifice_cover_bonus": 10.0,     # 牺牲掩护: 同归于尽为队友开路
        "step_penalty": -0.01,             # 步惩罚: 极轻
        "timeout_penalty": -50.0,          # 超时惩罚
        "smooth_action_coef": -0.003,      # 动作平滑: 更轻
        "altitude_penalty_coef": 8.0,      # 低高度保护: 强 (二次增长)
        "high_alt_penalty_coef": 3.0,      # 高高度惩罚: 防止无限爬升
        "spread_bonus_coef": 0.01,         # 分散阵型: 很轻
    },
